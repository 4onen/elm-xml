...
import Dict
import Xml.Decode.Advanced exposing (XmlValue,XmlContentValue(..))
...
xmlFileString dec str =
    case Parser.run (Xml.Decode.Advanced.xmlFile) str of
        Result.Ok xmlValue ->
            xmlValue
                |> dec
...
xmlString dec str =
    case Parser.run (Parser.succeed identity |. Parser.spaces |= Xml.Decode.Advanced.xml) str of
        Result.Ok xmlValue ->
            xmlValue
                |> dec
    ...
    case xmlContentValue of
        SubTags dict ->
            case Dict.get str dict of
                Just l ->
                    let
                        decodedL = List.map dec l
                        (firstError,succeedingL) = List.foldl
...
errorToString err =
    case err of
        ...
        BadRoot expected root ->
            "The expected root tag <"++expected++"> did not match the xml's real root tag <"++root++">.\n"
...
decodeParserDeadEnd de =
    let
        endInfo row col = " at row "++(String.fromInt row)++" and column "++(String.fromInt col)++".\n"
        expectation str row col = "I was expecting "++str++(endInfo row col)
    in
        case de.problem of
            Expecting str ->
